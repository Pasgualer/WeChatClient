...
class wxmessage
{
    ...
    public function run()
    {
        if (strtolower($_SERVER['REQUEST_METHOD']) == 'post') {  
            if ($this->_postData && $this->beforeProcess($this->_postData) === true) {
                ...
                if ($this->isTextMsg()) $Contentpost = $this->_postData->Content;
                else if ($this->isEventMsg()) $Contentpost = $this->_postData->Event;
                else $Contentpost = 'isTextMsg==0';
                $sql = sprintf("insert into text_record (Username, OpenId, CreateTime, MsgType, Content) values ('%s', '%s', '%s', '%s', '%s')",$this->_postData->ToUserName, $this->_postData->FromUserName, $date, $this->_postData->MsgType, $Contentpost);
                $mysql->sqlquery($sql);
            }
            else
    ...
    public function outputText($content)
    {
        ...
        /**
     * generate Image msg string
     * @param string $content
     * @return string xml
     */
    public function outputImage($content)
    {
        $textTpl = '<xml>
                <ToUserName><![CDATA[%s]]></ToUserName>
                <FromUserName><![CDATA[%s]]></FromUserName>
        ...
        $text = sprintf($textTpl, $this->_postData->FromUserName, $this->_postData->ToUserName, time(), self::REPLY_TYPE_IMAGE, $content);
        return $text;
    }
    /**
